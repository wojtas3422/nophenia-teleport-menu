...
func _ready() -> void:
	...
	var root_node = get_tree().get_first_node_in_group("teleport").get_node("teleport_ui")
	var search_bar = root_node.find_child("line_edit")
...
func _stage_to_first_result(new_text):
	...
	if first_location == null:
		return
	ModLoaderLog.info(first_location, self.name)
	_toggle_menu()
